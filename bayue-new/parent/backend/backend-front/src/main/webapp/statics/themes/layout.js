...
$(document).ready(function () {
	...
	$("#east-seperate .icon-v").click(function(){
		if(eastStatus===true){
			console.log("eastStatus is true");
			...
			console.log("eastStatus is false");
...
//		$(".east-menu").remove();
//		$("#east").append("<div class='east-menu'></div>");
//		var home_h2 = $("<h2><a href='javascript:void(0);'>菜单管理</a></h2><ul><li><a href=\"javascript:void(0);\" title='菜单管理' id=\"nav_rights_00\">菜单管理</a></li></ul>");
//		$(".east-menu").append(home_h2);
//		
//	});
	
	//左边菜单点击事件
//	$(".east-menu h2").live('click',function(){
	$("#east").on('click',"h2",function(){
...
//初始化系统菜单
function initMenu() {
	url = domain+"/loadAdminRights?m="+Math.random(); 
...
function initSubmenu() {
	...
	$.each(m.children, function (k, v) {		
		...
		if(m.children.length==1&&k==0){
			hidden = "";
		}
		var wElement=$("<div class=\"east-menu\"><h2><a href=\"javascript:void(0);\">" + v.text + "</a></h2><ul "+hidden+" class=\"bcw\"></ul></div>");	
...
//关闭指定选项卡
function closeTab(tabId,event,fn){
	//判断要关闭的tab是否是当前tab
	var tab = $("#"+tabId);
...
function showTab(tv){
	...
	if(tv.url.indexOf("linkId=") == -1 && tv.url.indexOf("tabId=") == -1){
		if(tv.url.indexOf("?")>0){
			tv.url = tv.url+"&linkId="+linkId+"&tabId="+tabId;
		}else{
			tv.url = tv.url+"?linkId="+linkId+"&tabId="+tabId;
		}
	}
