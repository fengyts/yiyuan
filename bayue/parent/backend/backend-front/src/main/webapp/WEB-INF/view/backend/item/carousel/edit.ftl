...
<form id="carouselEditForm" action="" class="form-horizontal dr-form-bordered">
	...
	<div class="display">
		<input type="hidden" id="id" name="id" value="${carouselDO.id}">
	</div>
	...
	<div class="form-group">
		...
		<div class="col-xs-8 radio">
			<label>
				<input type="radio" name="status" <#if carouselDO.status == 'true'>checked='checked'</#if> />有效
			</label>&nbsp;&nbsp;&nbsp;&nbsp;
			<label>
				<input type="radio" name="status" <#if carouselDO.status != 'true'>checked='checked'</#if> />无效
			</label>
		</div>
	</div>
	<div class="form-group">
